...
export function resolvePage (pages, rawPath, base) {
  ...
  for (let i = 0; i < pages.length; i++) {
    // console.log('path----', path, 'regularPath--', normalize(pages[i].regularPath))
...
export function filterPosts (posts, isTimeline) {
  posts = posts.filter((item, index) => {
    const { title, frontmatter: { home, date, publish, isNotTime } } = item
    // 过滤多个分类时产生的重复数据
    if (posts.indexOf(item) !== index) {
      return false
    } else {
      const someConditions = home == true || title == undefined || publish === false || isNotTime == true
